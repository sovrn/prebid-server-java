...
public class PrivacyEnforcementService {
    ...
    public PrivacyEnforcementService(RsidCookieService rsidCookieService,
    ...
    Future<PrivacyContext> contextFromBidRequest(
            BidRequest bidRequest, Account account, MetricName requestType, Timeout timeout, RoutingContext context) {
    ...
    public Future<PrivacyContext> contextFromSetuidRequest(
        ...
        return tcfDefinerService.resolveTcfContext(privacy, country, ipAddress, account.getGdpr(), timeout)
                .map(tcfContext -> PrivacyContext.of(privacy, tcfContext));
    }
